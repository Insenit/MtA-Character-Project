...
export default function ShifterPanel({ character, setCharacter, applyImmediateUpdate }) {
  const [expanded, setExpanded] = useState(false);
  const [proficiencyOverlayData, setProficiencyOverlayData] = useState(null);
  const classEntry = character.classes.find(c => c.name === 'Shifter');
  const shifterLevel = classEntry?.level || 1;
  const toggleExpanded = () => setExpanded(!expanded);

  const classPackage = classPackages.shifterPackage;
  ...
  return (
    ...
    >
      <div className="panel-header" onClick={toggleExpanded}>
        <h4 style={{ margin: 0 }}>Shifter Features</h4>
        <span style={{ marginRight: '6px' }}>{expanded ? '▾' : '▸'}</span>
      </div>

      {expanded && (
        <div style={{ marginTop: '10px' }}>
          {[...Array(shifterLevel)].map((_, i) => {
            const level = i + 1;
            const baseFeatures = ShifterFeatures[level] || [];
