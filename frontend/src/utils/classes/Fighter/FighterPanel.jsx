...
export default function FighterPanel({ character, setCharacter, applyImmediateUpdate }) {
  const [expanded, setExpanded] = useState(false);
  const [proficiencyOverlayData, setProficiencyOverlayData] = useState(null);
  const classEntry = character.classes.find(c => c.name === 'Fighter');
  const fighterLevel = classEntry?.level || 1;
  const toggleExpanded = () => setExpanded(!expanded);

  const classPackage = classPackages.fighterPackage;
  ...
  return (
    ...
    >
      <div className="panel-header" onClick={toggleExpanded}>
        <h4 style={{ margin: 0 }}>Fighter Features</h4>
        <span style={{ marginRight: '6px' }}>{expanded ? '▾' : '▸'}</span>
      </div>

      {expanded && (
        <div style={{ marginTop: '10px' }}>
          {[...Array(fighterLevel)].map((_, i) => {
            const level = i + 1;
            const baseFeatures = FighterFeatures[level] || [];
